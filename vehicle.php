<?php
    ...
    $obj->UserWall();
             ...
             $Data_R=$obj->myQuery("SELECT * FROM `tbl_vehicle` WHERE `business_id`='".$obj->Businessid($obj->UserKey)."'");
            //$Data_R=$obj->myQuery("select * from tbl_driver ORDER by banner_id DESC");
            if(isset($_GET["id"])){
    	$wh["vehicle_id"]=$_GET["id"];
    	$obj->myDelete("tbl_vehicle",$wh);
				...
				<div class="container">
					<div class="title-content">
						...
						<div class="breadcrumbs">
							...
							<span class="gt3_breadcrumb_divider"></span>
							<span class="current">VEHICLE</span>
						</div>
					</div>
				</div>
			</section>
                                    ...
                                    <tbody>
                                        ...
                                        while($res=$Data_R->fetch_assoc()) 
                                        {
											...
											$up_sub_category=$obj->myQuery("
									           ...
									           FROM `tbl_category` 
									           WHERE `category_id`='".$res['category_id']."' AND  `label`='sub'
									                          ")->fetch_assoc();
								            ...
								            $up_main_category=$obj->myQuery("
								                   ...
								                   WHERE s.`parentkey`=m.`category_id` AND s.`category_id`='".$up_sub_category['category_id']."' AND  m.`label`='main'
								                                  ")->fetch_assoc();
								            $driver=$obj->myQuery("SELECT * FROM `tbl_driver` WHERE  `driver_id`='".$res["driver_id"]."'")->fetch_assoc();	
                                        ?>
                                        <tr>
                                            <td><?php echo $no; ?></td>
                                            ...
                                            <td><?php echo $up_main_category["name"]; ?></td>
                                            <td><?php echo $up_sub_category["name"]; ?></td>
                                            <td><?php echo $res["plate_no"]; ?></td>
                                            <td><?php echo $res["weight"]; ?></td>
                                            <td><?php echo $driver["name"]; ?></td>
                                            <td><img src="<?php echo $obj->WebPath.$res["proof"]; ?>" class="zoom-img" width="100px"/></td>
                                            <td><a href="addvehicle.php?id=<?php echo $res["vehicle_id"]; ?>"><span  title="Update"><i style="font-size:23px;margin-left: 10px;" class="fa fa-pencil-square-o text-center" aria-hidden="true"></i>
                                                         </span></a>                                         
                                                 
                                                 <a href="vehicle.php?id=<?php echo $res["vehicle_id"]; ?>" onClick="return confirm('are you sure delete this record ?');"><span title="Delete"><i style="font-size:23px;" class="fa fa-trash-o" aria-hidden="true"></i>
                                                         </span></a></td>  
                                        </tr>
                                        <?php
                                        }
